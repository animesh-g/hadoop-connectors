 ...
 */
 ...
 class DeleteFolderOperation {
   ...
   private final MyStorageClient storageControlClient;
   ...
   DeleteFolderOperation(
       ...
       MyStorageClient storageControlClient) {
     ...
     this.batchExecutor = new BatchExecutor(100);
   ...
   public FolderInfo getElementFromBlockingQueue() throws InterruptedException {
     try {
       ...
       return folderDeleteBlockingQueue.poll(10, TimeUnit.SECONDS);
   ...
   protected synchronized void successfullDeletionOfFolderResource(FolderInfo folderResource) {
         this.success++;
         if (this.success % 1000 == 0) {
             System.out.println(String.format("%s of %s done", this.success, this.folders.size()));
           }
   ...
   private FutureCallback getDeletionCallback(
     ...
     return new FutureCallback<Void>() {
       ...
       public void onFailure(Throwable throwable) {
         System.out.println(String.format("%s - %s", resourceId, throwable));
   ...
   private class DeleteFolderRequestCallable implements Callable<Void> {
     ...
     private MyStorageClient storageControlClient;
     ...
     DeleteFolderRequestCallable(FolderInfo folder, MyStorageClient storageControlClient) {
